    ...
    def decorator(func):
        def wrappers(*args, **kwargs):
            ...
            if result in error_dict:
                error_message = f"В функции '{func.__name__[7::]}' возникла ошибка '{result}', входные данные: '{args[0]}'"
                ...
                log_message = f"Функция '{func.__name__[7::]}' выполнена за {end_time - start_time:.4f} секунд. Результат: {result}\n"
                if filename is not None:
                    with open(filename, 'a', encoding='utf-8') as f:
                        f.write(log_message)
                else:
