...
def test_logging_to_console_get_mask_card_number(capsys):
    ...
    assert (
        "Функция 'get_mask_card_number' выполнена за 0.0000 секунд. " "Результат: 7000 79** **** 6361" in captured.out
    )
...
def test_logging_to_file_get_mask_card_number():
    ...
    try:
        ...
        with open(temp_filename, "r", encoding="utf-8") as f:
            log_content = f.read()
        assert (
            "Функция 'get_mask_card_number' выполнена за 0.0000 секунд. Результат: 7000 79** **** 6361" in log_content
        )
    finally:
